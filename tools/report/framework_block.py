...
A multi-name note (a cluster or thematic comparison) names the stock on each marker instead,
and --body fills every one of them from the same scan in one run:

    <!--FRAMEWORK:BEGIN IOLCP-->
    <!--FRAMEWORK:END IOLCP-->

    python tools/report/framework_block.py --body tools/report/bodies/ibuprofen.html --apply
...
def block(code, row, prev_row, s2, s2date, label, universe_n, source_note, named=False):
    ...
    rows = [
        ...
        ('1-year return', fmt_pct(num(row.get('r1y'))), ''),
        ('6-month return', fmt_pct(num(row.get('r6m'))), ''),
    ...
    b, e = (f'<!--FRAMEWORK:BEGIN {code}-->', f'<!--FRAMEWORK:END {code}-->') if named else (BEGIN, END)
    return (f'{b}\n'
...
MARKER = re.compile(r'<!--FRAMEWORK:BEGIN(?:\s+([A-Z0-9&\-]+))?-->.*?<!--FRAMEWORK:END(?:\s+[A-Z0-9&\-]+)?-->', re.S)


def marker_codes(body):
    """Codes named on the body's markers; None stands for a bare (single-name) marker."""
    return [(m.group(1) or '').upper() or None for m in MARKER.finditer(body)]


def apply(path, blocks, default_code, cmp_label):
    """blocks: {code: block_html}. A bare marker takes default_code's block; a named marker its own.
    The masthead KPI cells are rewritten only for a single-name body (one bare marker)."""
    body = io.open(path, encoding='utf-8').read()
    codes = marker_codes(body)
    if not codes:
    ...
    def repl(m):
        nonlocal filled
        code = (m.group(1) or default_code or '').upper()
        blk = blocks.get(code)
        if not blk:
            missing.append(code or '?'); return m.group(0)
        filled += 1
        return blk
    new = MARKER.sub(repl, body)
    if codes == [None] and default_code and default_code in blocks:
        new, changed = kpi_rewrite(new, blocks_rows[default_code], cmp_label)
    io.open(path, 'w', encoding='utf-8').write(new)
    changed.append(f'{filled} block(s) filled' + (f'; no data for {", ".join(missing)}' if missing else ''))
    return body, changed
...
def main():
    ...
    ap.add_argument('--body', help='a specific body file to fill (for a multi-name note whose markers name '
                                  'their codes, e.g. <!--FRAMEWORK:BEGIN IOLCP-->); default is bodies/<code>.html')
    a = ap.parse_args()
    ...
    if a.all:
        for p in sorted(glob.glob(os.path.join(BODIES, '*.html'))):
            ...
            named = [c for c in marker_codes(b) if c]
            if named:
                codes += named
            elif BEGIN in b:
    ...
    if a.body and not codes:
        codes = [c for c in marker_codes(io.open(a.body, encoding='utf-8').read()) if c]
    if not codes:
        ap.error('name at least one NSE code, or pass --all')

    rc = 0
    if a.body:
        # one body, many stocks: every named marker gets its own block
        body_txt = io.open(a.body, encoding='utf-8').read()
        named = [c for c in marker_codes(body_txt) if c] or codes
        blocks = {}
        for code in named:
            row = rows.get(code)
            if not row:
                print(f'{code:<12} NOT in the {"snapshot" if a.snapshot else "scan"} — its marker is left as is'); rc = 1; continue
            blocks[code] = block(code, row, prev_rows.get(code), S2.get(code), s2date, label, universe_n, source_note, named=True)
        ...
        if a.apply:
            old, changed = apply(a.body, blocks, None, cmp_label)
            print(f'{os.path.relpath(a.body, ROOT)}: {"; ".join(changed)}')
        else:
            for code in named:
                if code in blocks: print(blocks[code]); print()
        return rc
    ...
    for code in codes:
        ...
        if a.apply:
            ...
            blocks_rows[code] = row
            old, changed = apply(body_path, {code: blk}, code, cmp_label)
            if old is None:
